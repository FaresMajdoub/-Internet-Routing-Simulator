 ...
 * \pre Le graphe doit être correctement initialisé et contenir au moins un sommet.
 * \post Retourne vrai si le graphe est fortement connexe, faux sinon.
 * \details Cette méthode utilise une approche similaire à l'algorithme de Kosaraju.
 * Elle réalise un parcours en profondeur (DFS) sur le graphe original, suivi
 * d'un parcours sur le graphe en inversant les arcs de manière implicite (sans construire
 * explicitement le graphe transposé). Cela permet de vérifier si tous les sommets
 * sont accessibles dans les deux directions.
 * @return Vrai si le graphe est fortement connexe, sinon faux.
 */
...
bool Graphe<Objet>::estFortementConnexe() const {
    ...
    std::list<Sommet*> visite; // liste de sommets visites
    int sommetDepart = m_listSommets->m_numero;

    _parcoursDFS(_getSommet(sommetDepart), visite);

    if (visite.size() != m_nbSommets) {
        return false;
    }
    // Verification supplementaire
    std::list<Sommet*> visiteTranspose;
    _initEtatSommets(false);
    std::stack<int> pile;
    ...
    while (!pile.empty()) {
        ...
        if (std::find(visiteTranspose.begin(), visiteTranspose.end(), _getSommet(sommet)) != visiteTranspose.end()) {
            continue;
        }

        visiteTranspose.push_back(_getSommet(sommet));
        Sommet* sommetIter = m_listSommets;

        while (sommetIter) {
            Arc* arc = sommetIter->m_listeDest;
            while (arc) {
                if (arc->m_dest->m_numero == sommet) {
                    pile.push(sommetIter->m_numero);
                }
                arc = arc->m_suivDest;
            }
            sommetIter = sommetIter->m_suivant;
        }
    }
    return visiteTranspose.size() == m_nbSommets;
}
/**
 * \brief Implémente l'algorithme de Dijkstra pour trouver le plus court chemin.
 * \pre Les sommets d'origine (\a p_eOrigine) et de destination (\a p_eDestination) doivent exister dans le graphe.
 * \pre Le graphe doit être correctement initialisé.
 * \post Le vecteur \a p_chemin contiendra les étiquettes du chemin le plus court.
 * \post Le graphe reste inchangé.
 * @return Le coût total du chemin le plus court entre les sommets d'origine et de destination.
 */
template<typename Objet>
int Graphe<Objet>::dijkstra(const Objet& p_eOrigine, const Objet& p_eDestination, std::vector<Objet>& p_chemin) {
    ...
    try {
        Sommet* sommetOrigine = _getSommet(getNumeroSommet(p_eOrigine));
        Sommet* sommetDestination = _getSommet(getNumeroSommet(p_eDestination));
        _initPathFinding(p_eOrigine, p_eDestination, sommetOrigine, sommetDestination);
        // Sauvegarder l'etat initial des sommets (pour les postconditions)
        std::vector<int> sommetsInitiaux = listerSommets();
        std::vector<Objet> etiquettesInitiales = listerEtiquetteSommets();

        std::vector<int> sommetsNonExplorés = listerSommets();

        while (!sommetsNonExplorés.empty()) {
            // Trouver le sommet avec le coût minimum dans sommetsNonExplorés
            Sommet* sommetCourant = nullptr;
            int coûtMin = std::numeric_limits<int>::max();
            for (int idSommet : sommetsNonExplorés) {
                Sommet* sommet = _getSommet(idSommet);
                if (sommet->m_cout < coûtMin) {
                    coûtMin = sommet->m_cout;
                    sommetCourant = sommet;
                }
            }
            // Retirer sommetCourant de sommetsNonExplorés
            sommetsNonExplorés.erase(
                std::remove(sommetsNonExplorés.begin(), sommetsNonExplorés.end(), sommetCourant->m_numero),
                sommetsNonExplorés.end()
            );
            // Parcourir les voisins de sommetCourant
            std::vector<int> sommetsAdjacents = listerSommetsAdjacents(sommetCourant->m_numero);
            for (int idVoisin : sommetsAdjacents) {
                int coûtTemporaire = sommetCourant->m_cout + getCoutArc(sommetCourant->m_numero, idVoisin);

                // Si un chemin plus court est trouve
                Sommet* sommetVoisin = _getSommet(idVoisin);
                if (coûtTemporaire < sommetVoisin->m_cout) {
                    sommetVoisin->m_cout = coûtTemporaire;
                    sommetVoisin->m_predecesseur = sommetCourant;
                }
            }
        }
        Sommet* sommetActuel = sommetDestination;
        if (sommetActuel->m_predecesseur != nullptr || sommetActuel == sommetOrigine) {
            // Construire le chemin en remontant les predecesseurs
            while (sommetActuel != nullptr) {
                p_chemin.insert(p_chemin.begin(), sommetActuel->m_etiquette);
                sommetActuel = sommetActuel->m_predecesseur;
            }
        } else {
            throw std::logic_error("Aucun chemin trouvé entre l'origine et la destination.");
        }

        POSTCONDITION(p_chemin.front() == p_eOrigine && p_chemin.back() == p_eDestination);
        POSTCONDITION(sommetsInitiaux == listerSommets());
        POSTCONDITION(etiquettesInitiales == listerEtiquetteSommets());

        POSTCONDITION(p_chemin.front() == p_eOrigine && p_chemin.back() == p_eDestination);
        return sommetDestination->m_cout;
    } catch (std::bad_alloc&) {
        std::cerr << "Erreur : allocation de mémoire insuffisante pour l'algorithme de Dijkstra." << std::endl;
        throw;
    }
}
/**
 * \brief Identifie les points d'articulation dans un graphe orienté.
 * @param p_sommets Vecteur où seront stockés les étiquettes des sommets d'articulation.
 * \throws std::bad_alloc si la mémoire est insuffisante.
 */
template <typename Objet>
void Graphe<Objet>::getPointsArticulation(std::vector<Objet>& p_sommets) {
    try {
        // Obtenir tous les sommets du graphe
        std::vector<int> sommets = this->listerSommets();

        for (unsigned int i = 0; i < sommets.size(); ++i) {
            int sommetCourant = sommets[i];
            bool estCritique = false;

            // Créer une copie du graphe sans le sommet courant
            Graphe<Objet> grapheCopie(*this);
            Objet etiquette = grapheCopie._getSommet(sommetCourant)->m_etiquette;
            grapheCopie.enleverSommet(sommetCourant);

            // Vérifier si la suppression du sommet crée des composantes isolées
            std::vector<int> sommetsRestants = grapheCopie.listerSommets();

            if (sommetsRestants.empty()) {
                continue;  // Ignorer si c'est le dernier sommet
            }

            // Prendre le premier sommet comme point de départ
            int sommetDepart = sommetsRestants[0];

            for (unsigned int j = 1; j < sommetsRestants.size() && !estCritique; ++j) {
                std::vector<Objet> chemin;
                try {
                    // Vérifier si on peut atteindre chaque sommet depuis le sommet de départ
                    Objet origine = grapheCopie._getSommet(sommetDepart)->m_etiquette;
                    Objet destination = grapheCopie._getSommet(sommetsRestants[j])->m_etiquette;
                    grapheCopie.dijkstra(origine, destination, chemin);

                    // Si on ne peut pas retourner au sommet de départ, le sommet est critique
                    std::vector<Objet> cheminRetour;
                    grapheCopie.dijkstra(destination, origine, cheminRetour);
                }
                catch (const std::logic_error&) {
                    estCritique = true;
                }

                // Vérifier les degrés entrant et sortant après la suppression
                if (!estCritique) {
                    std::vector<int> adjacents = grapheCopie.listerSommetsAdjacents(sommetsRestants[j]);
                    if (adjacents.empty()) {
                        estCritique = true;  // Le sommet devient isolé
                    }
                }
            }

            if (estCritique) {
                p_sommets.push_back(etiquette);
            }
        }
    }
    catch (const std::bad_alloc&) {
        std::cout << "Erreur : mémoire insuffisante pour retourner les points d'articulation." << std::endl;
        throw;
    }
}  